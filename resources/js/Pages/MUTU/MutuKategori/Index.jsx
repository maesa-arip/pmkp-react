...
export default function Index(props) {
    ...
    const openEditDialog = (MutuKategori) => {
        setState(MutuKategori);
        setIsOpenEditDialog(true);
    };
    const openDestroyDialog = (MutuKategori) => {
        setState(MutuKategori);
        setIsOpenDestroyDialog(true);
    };

    const destroyMutuKategori = () => {
        router.delete(route("MutuKategori.destroy", state.id), {
    ...
    return (
        <>
            <Head title="Mutu Kategori" />
            <AddModal
                isOpenAddDialog={isOpenAddDialog}
                setIsOpenAddDialog={setIsOpenAddDialog}
                size="max-w-4xl"
                title="Tambah Mutu Kategori"
            >
                <Create
                    isOpenAddDialog={isOpenAddDialog}
                    setIsOpenAddDialog={setIsOpenAddDialog}
            ...
            <DestroyModal
                ...
                size="max-w-4xl"
                title="Delete Mutu Kategori"
                warning="Yakin hapus data ini ?"
            >
                <DangerButton className="ml-2" onClick={destroyMutuKategori}>
                    Delete
                </DangerButton>
            </DestroyModal>
            <div className="px-2 py-12 bg-white border rounded-xl">
                <div className="mx-auto sm:px-6 lg:px-8">
                    ...
                    <Table overflow="none">
                        ...
                        <Table.Tbody>
                            {MutuKategori.map((MutuKategori, index) => (
                                <tr key={index}>
                                    <Table.Td>

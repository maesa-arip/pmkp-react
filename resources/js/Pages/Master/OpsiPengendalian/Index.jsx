...
export default function Index(props) {
    const {
        data: opsiPengendalian,
        meta,
        filtered,
        attributes,
    } = props.opsiPengendalian;
    ...
    const openEditDialog = (opsiPengendalian) => {
        setState(opsiPengendalian);
        setIsOpenEditDialog(true);
    };
    const openDestroyDialog = (opsiPengendalian) => {
        setState(opsiPengendalian);
        setIsOpenDestroyDialog(true);
    };

    const destroyopsiPengendalian = () => {
        router.delete(route("opsiPengendalians.destroy", state.id), {
    ...
    return (
        <>
            <Head title="Opsi Pengndalian" />
            <AddModal
                isOpenAddDialog={isOpenAddDialog}
                setIsOpenAddDialog={setIsOpenAddDialog}
                size="max-w-4xl"
                title="Tambah Opsi Pengndalian"
            >
                <Create
                    isOpenAddDialog={isOpenAddDialog}
                    setIsOpenAddDialog={setIsOpenAddDialog}
            ...
            <DestroyModal
                ...
                size="max-w-4xl"
                title="Delete Opsi Pengendalian"
                warning="Yakin hapus data ini ?"
            >
                <DangerButton className="ml-2" onClick={destroyopsiPengendalian}>
                    Delete
                </DangerButton>
            </DestroyModal>
                                ...
                                <div className="border-b border-gray-200 shadow sm:rounded-lg">
                                    <table className="min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {opsiPengendalian.map(
                                                (opsiPengendalian, index) => (
                                                    <tr key={index}>
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            {meta.from + index}
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            {opsiPengendalian.name}
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            {opsiPengendalian.joined}
                                                        </td>
                                                        <td>
                                                            <Dropdown>
                                                                <Dropdown.Trigger>

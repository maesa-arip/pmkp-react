...
export default function Index(props) {
    ...
    const [selectedRows, setSelectedRows] = useState([]);
        ...
        const isSelected = (riskregisterklinis1) => {
          return selectedRows.some(
            (selectedItem) => selectedItem.id === riskregisterklinis1.id
          );
        };

        const toggleSelected = (riskregisterklinis1) => {
          const index = selectedRows.findIndex(
            (selectedItem) => selectedItem.id === riskregisterklinis1.id
          );
          if (index === -1) {
            setSelectedRows([...selectedRows, riskregisterklinis1]);
          } else {
            setSelectedRows(
              selectedRows.filter((selectedItem) => selectedItem.id !== riskregisterklinis1.id)
            );
          }
        };

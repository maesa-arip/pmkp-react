...
export default function Form({
    ...
    // Auto-generate Pernyataan Risiko sesuai aslinya
    useEffect(() => {
    ...
    const inputClass = "block w-full text-sm font-medium text-slate-900 bg-white border border-slate-300 rounded-lg dark:text-slate-100 dark:bg-[#0f172a] dark:border-slate-700 focus:bg-white dark:focus:bg-[#020817] focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 outline-none transition-all shadow-sm placeholder:text-slate-400";
    ...
    return (
        <div className="relative flex flex-col w-full h-full bg-slate-50/30 dark:bg-transparent">
            ...
            <div className="flex-1 p-4 space-y-6 overflow-y-auto sm:p-6 custom-scrollbar">
                ...
                <div className={`${sectionCardClass} overflow-hidden`}>
                    ...
                    <div className="p-6 pl-8">
                        <div className="flex items-center text-[10px] font-black uppercase tracking-widest text-teal-600 dark:text-teal-400 mb-2">
                            <DocumentTextIcon className="w-4 h-4 mr-1.5" />
                            Data Pernyataan Risiko Manajemen
                        </div>
                        <TextAreaInput
                ...
                {/* --- SECTION 3: VERIFIKASI MANAJEMEN (FORM INPUT) --- */}
                <div className={`${sectionCardClass} border-teal-200 dark:border-teal-500/30 overflow-hidden mb-4`}>
                    <div className="px-6 py-5 border-b border-teal-100 bg-teal-50/50 dark:bg-teal-500/5 dark:border-teal-500/20">
                        <div className="flex items-center">
                            <ShieldCheckIcon className="w-6 h-6 mr-2 text-teal-600 dark:text-teal-400" />
                            <div>
                                <h3 className="text-base font-bold text-slate-900 dark:text-white">Verifikasi Manajemen IKP</h3>
                                <p className="mt-0.5 text-xs font-medium text-slate-500 dark:text-slate-400">Masukkan catatan persetujuan manajemen dan tentukan status akhir.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-6 p-6 md:grid-cols-12 bg-white dark:bg-[#0f172a]">
                        <div className="relative flex flex-col col-span-12 z-[40]">
                            <label className={labelClass}>Keterangan / Catatan Verifikasi Manajemen</label>
                            <TextAreaInput
                                id="keterangan"
                                value={data.keterangan}
                                handleChange={(e) => setData("keterangan", e.target.value)}
                                rows={4}
                                className={`${inputClass} focus:border-teal-500 focus:ring-teal-500/20`}
                                placeholder="Masukkan catatan hasil pengawasan manajemen..."
                            />
            ...
            <div className="shrink-0 p-4 sm:p-6 bg-white dark:bg-[#0f172a] border-t border-slate-200 dark:border-slate-800/80 flex flex-col sm:flex-row-reverse justify-start gap-3 mt-auto z-[90] rounded-b-2xl">
                <button 
                    type="submit" 
                    className="w-full sm:w-auto inline-flex justify-center items-center px-8 py-2.5 text-sm font-bold text-white transition-colors bg-teal-600 rounded-xl shadow-sm hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500/50"
                >
                    {submit}
                </button>
                <button 

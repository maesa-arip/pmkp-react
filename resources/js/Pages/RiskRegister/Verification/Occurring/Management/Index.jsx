...
export default function Index(props) {
    ...
    return (
        <div className="relative min-h-screen p-0 font-sans bg-transparent dark:bg-transparent text-slate-900 dark:text-slate-100 sm:p-2">
            <Head title="Verifikasi Manajemen Risiko" />
            
            <EditModal isOpenEditDialog={isOpenVerification} setIsOpenEditDialog={setIsOpenVerification} size="max-w-6xl" title="Verifikasi Manajemen: Risiko Sedang Terjadi">
                ...
                <div className="bg-white dark:bg-[#0f172a] border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-6 shadow-sm relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex flex-col justify-between gap-5 md:flex-row md:items-end">
                        <div className="space-y-1.5">
                            ...
                            <h2 className="text-2xl font-black tracking-tight text-slate-900 dark:text-slate-50">Verifikasi Manajemen IKP</h2>
                            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Persetujuan dan verifikasi akhir tingkat manajemen untuk laporan perbaikan insiden.</p>
                        </div>
                    ...
                    {riskRegisterKlinis.length === 0 ? (
                        <div className="absolute inset-0 z-0 flex flex-col items-center justify-center p-6 text-center">
                            ...
                            <p className="max-w-sm mt-2 text-sm font-medium text-slate-500 dark:text-slate-400">
                                {params.q ? "Pencarian Anda tidak menemukan hasil yang cocok." : "Belum ada risiko sedang terjadi yang memerlukan verifikasi manajemen."}
                            </p>
                        </div>
                    ) : (
                            ...
                            <table className="w-full text-sm text-left border-collapse whitespace-nowrap min-w-max">
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/80">
                                    {riskRegisterKlinis.map((item, index) => {
                                        ...
                                        // Gunakan verificationpriorityadmin seperti kode asli jika verificationmanagement belum ada
                                        ...
                                        return (
                                            <tr 
                                                ...
                                                className={`group transition-colors cursor-pointer ${isSelected ? "bg-teal-50/50 dark:bg-white/[0.04]" : "bg-white dark:bg-[#0f172a] hover:bg-slate-50/80 dark:hover:bg-[#161f33]"}`}
                                            >
                                                <td className={`px-5 py-4 text-center border-r border-slate-100 dark:border-slate-800/80 align-middle sticky left-0 bg-clip-padding ${isSelected ? "bg-teal-50 dark:bg-[#1e293b]" : "bg-white dark:bg-[#0f172a] group-hover:bg-slate-50 dark:group-hover:bg-[#161f33]"} shadow-[4px_0_10px_-4px_rgba(0,0,0,0.05)] z-10`}>
                                                ...
                                                <td className="px-5 py-4 border-r border-slate-100 dark:border-slate-800/80 align-top whitespace-normal max-w-[250px]">
                                                    ...
                                                    <div className="pt-2 mt-auto border-t border-slate-100 dark:border-slate-800/80">
                                                        ...
                                                        <span className="text-xs font-bold text-teal-700 dark:text-teal-400">{item.user?.name || '-'}</span>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="px-5 py-4 text-center align-top">
                                                    <div className="relative inline-block text-left" onClick={(e) => e.stopPropagation()}>
                                                        <button onClick={() => toggleDropdown(item.id)} className="p-2 transition-colors rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 dark:hover:text-slate-200 dark:hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500/50">
                                                        ...
                                                        {openDropdownId === item.id && (
                                                            <div className="absolute right-0 z-[100] w-48 mt-2 origin-top-right bg-white border border-slate-200 rounded-xl shadow-lg dark:bg-[#1e293b] dark:border-slate-700 ring-1 ring-black ring-opacity-5 focus:outline-none divide-y divide-slate-100 dark:divide-slate-700/80">
                                                                <div className="py-1">
                                                                    <button onClick={() => { openVerification(item); setOpenDropdownId(null); }} className="flex items-center w-full px-4 py-2.5 text-xs font-bold text-teal-600 transition-colors hover:bg-teal-50 dark:hover:bg-teal-500/10 dark:text-teal-400 group">
                ...
                <>
                    ...
                    <div className="fixed inset-y-2 right-2 sm:right-4 z-50 w-[calc(100%-1rem)] sm:w-[500px] bg-white dark:bg-[#0f172a] shadow-2xl border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col transition ease-in-out animate-slide-in overflow-hidden">
                        ...
                        <div className="flex items-center justify-between px-6 py-5 bg-white border-b border-slate-100 dark:border-slate-800 dark:bg-[#0f172a]">
                            <div>
                                <h2 className="text-lg font-black tracking-tight text-slate-900 dark:text-white">Detail Verifikasi Manajemen</h2>
                        ...
                        <div className="flex-1 p-6 space-y-6 overflow-y-auto custom-scrollbar bg-slate-50/50 dark:bg-transparent">
                            ...
                            <section className="p-5 bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm relative overflow-hidden">
                                ...
                                <label className="text-[10px] font-black uppercase text-teal-600 dark:text-teal-400 tracking-widest block mb-2 pl-2">Konteks Pernyataan Risiko</label>
                            ...
                            <section className="p-5 border border-teal-100 shadow-sm bg-teal-50/50 dark:bg-teal-500/5 dark:border-teal-500/20 rounded-2xl">
                                <label className="text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400 tracking-widest block mb-3 border-b border-teal-200 dark:border-teal-500/30 pb-2">Verifikasi Manajemen</label>
                                <div className="space-y-4">
                                    <div>
                                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 block mb-1">Keterangan Verifikasi Manajemen</span>
                                        <p className="text-sm font-medium leading-relaxed text-slate-800 dark:text-slate-300">
                                            {state.requestupdateverificationmanagement?.keterangan || <span className="italic text-slate-400">Belum diverifikasi.</span>}
                                        </p>
                                    </div>
                                </div>
                            </section>
                        ...
                        <div className="flex flex-col gap-3 p-5 bg-white border-t shrink-0 border-slate-100 dark:border-slate-800 dark:bg-[#0f172a]">
                             <button onClick={() => openVerification(state)} className="flex items-center justify-center w-full py-2.5 bg-teal-600 hover:bg-teal-700 text-white text-sm font-bold rounded-xl transition-colors shadow-sm focus:ring-2 focus:ring-teal-500/50">
                                <CheckBadgeIcon className="w-4 h-4 mr-2" /> Lakukan Verifikasi Manajemen
                             </button>

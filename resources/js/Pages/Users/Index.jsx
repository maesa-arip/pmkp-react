    ...
    return (
        <div className="relative min-h-screen p-0 font-sans bg-transparent text-slate-900 dark:bg-transparent dark:text-slate-100 sm:p-2">
            ...
            <AddModal isOpenAddDialog={isOpenAddDialog} setIsOpenAddDialog={setIsOpenAddDialog} size="max-w-4xl" title="Tambah Pengguna Baru">
                <Create roles={roles} pics={pics} locations={locations} enabled={enabled} setEnabled={setEnabled} isOpenAddDialog={isOpenAddDialog} setIsOpenAddDialog={setIsOpenAddDialog} />
            </AddModal>
            
            <EditModal isOpenEditDialog={isOpenEditDialog} setIsOpenEditDialog={setIsOpenEditDialog} size="max-w-4xl" title="Edit Pengguna">
                <Edit roles={roles} pics={pics} locations={locations} model={state} isOpenEditDialog={isOpenEditDialog} setIsOpenEditDialog={setIsOpenEditDialog} />
            </EditModal>
                            ...
                            <table className="w-full text-sm text-left border-collapse whitespace-nowrap min-w-max">
                                <thead className="bg-slate-50 dark:bg-[#09090b]">
                                    <tr>
                                        ...
                                        <th className="px-5 py-4 border-b border-r border-slate-200 dark:border-slate-800/80 w-[240px]">
                                    ...
                                    {people.map((person, index) => {
                                        ...
                                        return (
                                            <tr key={person.email} onClick={() => onSelectRow(index)} className={`group transition-colors duration-200 cursor-pointer ${isSelected ? "bg-blue-50/50 dark:bg-white/[0.04]" : "bg-white dark:bg-[#0f172a] hover:bg-slate-50/80 dark:hover:bg-[#161f33]"}`}>
                                                ...
                                                <td className="px-5 py-4 align-middle border-r border-slate-100 dark:border-slate-800/80">
                                                    {person.pic?.name ? (
                                                        <span className="inline-flex max-w-[220px] items-center rounded-md border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700 shadow-sm dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300">
                                                            <span className="truncate">{person.pic.name}</span>
                                                        </span>
                                                    ) : (
                                                        <span className="text-[11px] italic text-slate-400">Belum memilih unit</span>
                                                    )}
                                                </td>
                ...
                <>
                    ...
                    <div className="fixed inset-y-2 right-2 sm:right-4 z-50 w-[calc(100%-1rem)] sm:w-[420px] bg-white dark:bg-[#0f172a] shadow-2xl border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col transition ease-in-out animate-slide-in overflow-hidden">
                        ...
                        <div className="flex-1 p-6 overflow-y-auto space-y-7 custom-scrollbar bg-slate-50/50 dark:bg-transparent">
                            ...
                            <section>
                                <label className="text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400 tracking-widest block mb-3 border-b border-slate-200 dark:border-slate-800 pb-2">Unit / PIC</label>
                                <div className="inline-flex max-w-full items-center px-4 py-2 bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm text-sm font-bold text-slate-800 dark:text-slate-200">
                                    <span className="truncate">{state.pic?.name || "Belum memilih unit"}</span>
                                </div>
                            </section>

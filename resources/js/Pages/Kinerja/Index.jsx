...
import { ArrowDownTrayIcon, ArrowPathIcon, CalendarDaysIcon, ChevronLeftIcon, ChevronRightIcon, ExclamationTriangleIcon, FolderOpenIcon, LinkIcon, LockClosedIcon, MagnifyingGlassIcon, PencilSquareIcon, PlusIcon, Squares2X2Icon, XMarkIcon } from '@heroicons/react/24/outline';
...
export default function Index({ periods, period, nodes, locations, exports, cascadingTree = [], cascadingConcepts = [], cascadingIssues = [], unplacedIndicators = 0, responsiblePositions = [], pics = [], performanceIndicators = [] }) {
    ...
    useEffect(() => { setPage(1); }, [level, search, showInactive]);
    ...
    // Only the direct parent is picked; the levels above it follow from that choice.
    const parentLevel = parents[level] ? String(Number(level) - 1) : null;
    const selectedParent = parentLevel ? (nodes[parentLevel] || []).find(row => String(row.id) === String(node.data[parents[level][0]])) : null;
    const parentAncestors = [];
    for (let ancestorLevel = Number(parentLevel) - 1, child = selectedParent; ancestorLevel >= 1; ancestorLevel--) {
        child = child ? (nodes[ancestorLevel] || []).find(row => String(row.id) === String(child['indikator_fitur' + ancestorLevel + '_id'])) : null;
        parentAncestors.unshift([String(ancestorLevel), child]);
    }
    ...
    // A unit indicator takes the position of its activity; only a new indicator picks one.
    const followsActivity = !!editingRow && (!editingRow.penanggung_jawab_id || String(editingRow.penanggung_jawab_id) === String((nodes['3'] || []).find(row => String(row.id) === String(editingRow.indikator_fitur3_id))?.penanggung_jawab_id));
    const responsibleId = followsActivity ? selectedParent?.penanggung_jawab_id : node.data.penanggung_jawab_id;
    const responsible = responsiblePositions.find(position => String(position.id) === String(responsibleId));
    ...
    // Level four keeps inactive masters (e.g. the deactivated KATIM rows) out of view unless asked.
    const levelRows = id => (nodes[id] || []).filter(x => id !== '4' || showInactive || x.is_active);
    const inactiveCount = (nodes['4'] || []).filter(x => !x.is_active).length;
    const rows = levelRows(level).filter(x => `${x.name} ${x.jabatan || ''} ${x.display_code || x.kode_cascading || ''} ${x.id}`.toLowerCase().includes(search.toLowerCase()));
    ...
    // A level-four master linked to the year but never positioned under an activity.
    const unlinked = item => level === '4' && !item.indikator_fitur3_id;
    ...
    const levelCount = id => strategicLevels[id] ? cascadingConcepts.filter(item => item.kind === id).length : levelRows(id).length;
    const indicatorCount = levelOrder.reduce((sum, key) => sum + levelRows(key).length, 0);
        ...
        {showMaster && <ResponsiblePositions positions={responsiblePositions} locations={locations} pics={pics} />}
            {editing && canEdit && <KinerjaModal show={editing} busy={node.processing} onClose={() => setEditing(false)} title={(node.data.id ? 'Edit ' : 'Tambah ') + levels[level].toLowerCase()} description={'Perubahan disimpan pada ' + (isDraft ? 'draft' : 'periode aktif') + ' tahun ' + period.tahun + '.'}><form className="space-y-5" onSubmit={e => { e.preventDefault(); node.post(route('kinerja.nodes', [period.id, level]), { preserveScroll: true, onSuccess: () => setEditing(false) }); }}>
                <div className="grid gap-3 md:grid-cols-2">
                    ...
                    {parents[level] && <fieldset className="min-w-0 space-y-3 rounded-xl border border-slate-200 p-4 dark:border-slate-700 md:col-span-2"><legend className="px-1 text-sm font-semibold">Indikator induk</legend>
                        {parentAncestors.map(([id, ancestor]) => <Field key={id} label={levels[id]}><textarea readOnly rows={2} className={`${input} bg-slate-50 dark:bg-slate-800/50`} placeholder="Mengikuti induk yang dipilih" value={ancestor ? `${ancestor.display_code || ancestor.kode_cascading || '#' + ancestor.id} — ${ancestor.name}` : ''} /></Field>)}
                        <Field label={levels[parentLevel] + ' *'}><KinerjaSelect value={node.data[parents[level][0]] || ''} onChange={value => node.setData(parents[level][0], value)} options={options((nodes[parentLevel] || []).filter(item => isDraft || item.is_active))} /></Field>
                        <p className={muted}>Cukup pilih {levels[parentLevel].toLowerCase()}; induk di atasnya otomatis mengikuti pilihan itu.</p>
                    </fieldset>}
                    {level === '4' && <section aria-label="PIC dan unit yang dapat memakai indikator" className="min-w-0 rounded-xl bg-sky-50 p-4 dark:bg-sky-900/20 md:col-span-2"><h4 className="text-sm font-semibold">PIC dan unit yang dapat memakai indikator</h4><p className={muted}>{editingRow ? 'Unit indikator tetap sama di setiap tahun dan tidak berubah saat penanggung jawab diganti.' : 'PIC jabatan pemilik indikator dan unit pelaksana yang diatur pada master dapat memakai indikator ini.'}</p>{!followsActivity && responsible?.pic_id && <p className="mt-2 text-sm font-medium text-sky-700 dark:text-sky-300">PIC jabatan: {responsible.name}</p>}{responsibleUnits.length ? <ul className="mt-3 grid gap-2 sm:grid-cols-2">{responsibleUnits.map(unit => <li key={unit.id} className="rounded-lg border border-sky-100 bg-white px-3 py-2 text-sm dark:border-sky-900 dark:bg-slate-900">{unit.name}</li>)}</ul> : <p className="mt-3 text-sm font-medium text-amber-700 dark:text-amber-400">{editingRow ? (savedUnits.includes(0) ? 'Berlaku untuk semua unit.' : 'Unit indikator belum diatur.') : node.data.penanggung_jawab_id ? (responsible?.pic_id ? 'Belum ada tambahan unit pelaksana.' : 'Hubungkan PIC jabatan atau lengkapi unit pelaksana sebelum menyimpan.') : 'Pilih penanggung jawab untuk menampilkan unit.'}</p>}</section>}
                </div>

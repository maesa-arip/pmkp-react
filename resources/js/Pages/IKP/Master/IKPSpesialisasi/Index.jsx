...
export default function Index(props) {
    const {
        data: IkpSpesialisasi,
        meta,
        filtered,
        attributes,
    } = props.IkpSpesialisasi;
    ...
    const openEditDialog = (IkpSpesialisasi) => {
        setState(IkpSpesialisasi);
        setIsOpenEditDialog(true);
    };
    const openDestroyDialog = (IkpSpesialisasi) => {
        setState(IkpSpesialisasi);
        setIsOpenDestroyDialog(true);
    };

    const destroyIkpSpesialisasi = () => {
        router.delete(route("IkpSpesialisasi.destroy", state.id), {
    ...
    return (
        <>
            ...
            >
                <DangerButton className="ml-2" onClick={destroyIkpSpesialisasi}>
                    Delete
                </DangerButton>
            </DestroyModal>
            <div className="px-2 py-12 bg-white border rounded-xl">
                <div className="mx-auto sm:px-6 lg:px-8">
                    ...
                    <Table overflow="none">
                        ...
                        <Table.Tbody>
                            {IkpSpesialisasi.map((IkpSpesialisasi, index) => (
                                <tr key={index}>
                                    <Table.Td>
                                        <Badge>{meta.from + index}</Badge>
                                    </Table.Td>
                                    <Table.Td className="whitespace-nowrap">
                                        {IkpSpesialisasi.name}
                                    </Table.Td>
                                    <Table.Td>
                                        {IkpSpesialisasi.joined}
                                    </Table.Td>
                                    <Table.Td>
                                        <Dropdown>
                                            <Dropdown.Trigger>

...
export default function Index(props) {
    const {
        data: IkpJenisInsiden,
        meta,
        filtered,
        attributes,
    } = props.IkpJenisInsiden;
    ...
    const openEditDialog = (IkpJenisInsiden) => {
        setState(IkpJenisInsiden);
        setIsOpenEditDialog(true);
    };
    const openDestroyDialog = (IkpJenisInsiden) => {
        setState(IkpJenisInsiden);
        setIsOpenDestroyDialog(true);
    };

    const destroyIkpJenisInsiden = () => {
        router.delete(route("IkpJenisInsidens.destroy", state.id), {
    ...
    return (
        <>
            <Head title="IKP Jenis Insiden" />
            <AddModal
                isOpenAddDialog={isOpenAddDialog}
                setIsOpenAddDialog={setIsOpenAddDialog}
                size="max-w-4xl"
                title="Tambah IKP Jenis Insiden"
            >
                <Create
                    isOpenAddDialog={isOpenAddDialog}
                    setIsOpenAddDialog={setIsOpenAddDialog}
            ...
            <DestroyModal
                ...
                size="max-w-4xl"
                title="Delete IKP Jenis Insiden"
                warning="Yakin hapus data ini ?"
            >
                <DangerButton className="ml-2" onClick={destroyIkpJenisInsiden}>
                    Delete
                </DangerButton>
            </DestroyModal>
            <div className="px-2 py-12 bg-white border rounded-xl">
                <div className="mx-auto sm:px-6 lg:px-8">
                                ...
                                <div className="border-b border-gray-200 shadow sm:rounded-lg">
                                    <table className="min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {IkpJenisInsiden.map(
                                                (IkpJenisInsiden, index) => (
                                                    <tr key={index}>
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            {meta.from + index}
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            {IkpJenisInsiden.name}
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            {IkpJenisInsiden.joined}
                                                        </td>
                                                        <td>
                                                            <Dropdown>
                                                                <Dropdown.Trigger>
                    ...
                    <Table overflow="none">
                        ...
                        <Table.Tbody>
                            {IkpJenisInsiden.map((IkpJenisInsiden, index) => (
                                <tr key={index}>
                                    <Table.Td>

...
export default function Index(props) {
    const {
        data: IkpProbabilitas,
        meta,
        filtered,
        attributes,
    } = props.IkpProbabilitas;
    ...
    const openEditDialog = (IkpProbabilitas) => {
        setState(IkpProbabilitas);
        setIsOpenEditDialog(true);
    };
    const openDestroyDialog = (IkpProbabilitas) => {
        setState(IkpProbabilitas);
        setIsOpenDestroyDialog(true);
    };

    const destroyIkpProbabilitas = () => {
        router.delete(route("IkpProbabilitas.destroy", state.id), {
    ...
    return (
        <>
            ...
            >
                <DangerButton className="ml-2" onClick={destroyIkpProbabilitas}>
                    Delete
                </DangerButton>
            </DestroyModal>
            <div className="px-2 py-12 bg-white border rounded-xl">
                <div className="mx-auto sm:px-6 lg:px-8">
                    ...
                    <Table overflow="none">
                        ...
                        <Table.Tbody>
                            {IkpProbabilitas.map((IkpProbabilitas, index) => (
                                <tr key={index}>
                                    <Table.Td>
                                        <Badge>{meta.from + index}</Badge>
                                    </Table.Td>
                                    <Table.Td className="whitespace-nowrap">
                                        {IkpProbabilitas.value}
                                    </Table.Td>
                                    <Table.Td className="whitespace-nowrap">
                                        {IkpProbabilitas.name}
                                    </Table.Td>
                                    <Table.Td>
                                        {IkpProbabilitas.joined}
                                    </Table.Td>
                                    <Table.Td>
                                        <Dropdown>
                                            <Dropdown.Trigger>

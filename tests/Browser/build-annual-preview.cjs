...
async function main() {
    ...
    const out = await esbuild.build({
        stdin: { contents: `
            ...
            if (params.get('case') === 'single') tree[0].children = tree[0].children.slice(0, 1);
            if (params.get('case') === 'mixed') {
                tree[0].children[0].name = 'Peningkatan kualitas pelayanan dan keselamatan pasien melalui penguatan kompetensi serta evaluasi berkelanjutan seluruh unit pelayanan';
                tree[0].children[0].children.push({id:24,level:'3',display_code:'1.1.b',name:'Kegiatan kedua dengan dua indikator',children:[{id:41,level:'4',display_code:'1.1.b.1',name:'Kepatuhan pelaporan',children:[]},{id:42,level:'4',display_code:'1.1.b.2',name:'Ketepatan waktu tindak lanjut',children:[]}]});

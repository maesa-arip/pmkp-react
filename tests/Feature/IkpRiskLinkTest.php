...
class IkpRiskLinkTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->risk = $risk;
        // Keep this fixture available to all units; the production annual observer
        // now rejects assigning a unit-specific indicator to an unrelated PIC.
        \Illuminate\Support\Facades\DB::table('indikator_fitur4s')->where('id', $risk->indikator_fitur4_id)->update(['location_id' => '[0]']);
        $this->risk->update(['pic_id' => (string) $unit->id, 'currently_id' => 2]);
        $this->actingAs(User::factory()->create(['pic_id' => $unit->id]));
